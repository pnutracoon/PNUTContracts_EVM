...
import "../../openzeppline/v4/@openzeppelin/contracts/utils/Strings.sol";
import "../../openzeppline/v4/@openzeppelin/contracts/utils/introspection/ERC165.sol";
    ...
    ERC165
{
    ...
    function _removeToken(uint256 tokenId) internal virtual {
        ...
        require(
            index < _indexedTokenIds[token.owner].length,
            "Trying to delete out of bound index"
        );

        if (index != _indexedTokenIds[token.owner].length - 1) {
            uint256 newTokenId = _indexedTokenIds[token.owner][
                _indexedTokenIds[token.owner].length - 1
            ];
